...
std::vector<Kuplung_DX::Models::MeshModel> Kuplung_DX::Importers::ObjParser::parse(const std::string& fileToParse, const std::vector<std::string>& settings) {
	...
	if (!ifs.is_open()) {
		...
		app.LogInfo(Kuplung_DX::Utilities::CXXUtils::StringToPlatformString(msg));
		ifs.close();
		return {};
	}
...
//}
//
//MeshMaterialTextureImage Kuplung_DX::Importers::ObjParser::parseTextureImage(const std::string& textureLine) {
