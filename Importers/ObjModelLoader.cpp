#include "pch.h"
#include "Importers\ObjModelLoader.h"

using namespace Kuplung_DX::Importers;

bool ObjModelLoader::LoadObjModel(ID3D11Device* device, std::wstring Filename, ObjModel& Model, std::vector<SurfaceMaterial>& material, TextureManager& TexMgr, bool IsRHCoordSys, bool ComputeNormals, bool flipFaces) {
	HRESULT hr = 0;
						...
						if (checkChar == '_')
						{
							...
							if (checkChar == 'K')
							{
								...
								if (checkChar == 'd')
								{
									...
									//	hr = DirectX::D3DX11CreateShaderResourceViewFromFile(device, fileNamePath.c_str(), NULL, NULL, &tempSRV, NULL);
									//	if (SUCCEEDED(hr)) {
								...
								if (checkChar == 'a')
								{
									...
									//{
									//	ID3D11ShaderResourceView* tempSRV;
									//	hr = D3DX11CreateShaderResourceViewFromFile(device, fileNamePath.c_str(), NULL, NULL, &tempSRV, NULL);
									//	if (SUCCEEDED(hr))
									//	{
									//		TexMgr.TextureNameArray.push_back(fileNamePath.c_str());
